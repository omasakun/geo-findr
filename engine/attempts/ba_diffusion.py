...
class GeoModule(BaseLightningModule):
  def __init__(self, config: DotDict):
    ...
    hdim = self.vit.config.hidden_size + 4
    self.head = torch.nn.Sequential(  # TODO: better head
        torch.nn.Linear(hdim, hdim),
        torch.nn.ReLU(),
        torch.nn.Linear(hdim, 256),
        torch.nn.ReLU(),
  ...
  def forward_diffusion(self, vit_features: Tensor, noise_t: Tensor | int, xt: Tensor):
    ...
    combined_input = torch.cat([vit_features, noise_t, xt], dim=1)
    return self.head(combined_input)
